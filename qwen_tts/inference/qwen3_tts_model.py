...
class Qwen3TTSModel:
    ...
    def _load_local_audio_to_np(self, path: str) -> Tuple[np.ndarray, int]:
        try:
        ...
        with audioread.audio_open(path) as reader:
            ...
            audio = np.concatenate(pcm_chunks, axis=0)
            if channels > 1:
    ...
    def _load_audio_to_np(self, x: str) -> Tuple[np.ndarray, int]:
        if self._is_url(x):
            ...
            audio, sr = self._load_local_audio_to_np(x)
    ...
    def generate_voice_clone(
        ...
        **kwargs,
        ...
        Args:
            ...
            instruct:
                Optional instruction(s) describing desired voice/style traits for generation.
            x_vector_only_mode:
